...
def save_best_loss_model(model_name, model, best_loss):
    ...
    torch.save(model, base_dir + 'playground/ssupervae_intro/weights/' + model_name + ".pth")


def train(data_loader,
          config,
          model_name='ssupervae_intor',
          cont_epoch=-1,
          cont_model=None):
    # loading config
    ...
    optimizer_e = optim.Adam(list(net.encoder.parameters()) + list(net.face_encoder.parameters()),
                             lr=config.lr,
    ...
    save_training_plot(train_losses['loss'],
                       ...
                       base_dir + 'playground/ssupervae_intro/' + f'results/ssupervae_intro_plot.png'
                       )
    return net
